...
# Flamegraph
export PATH=$PATH:$HOME/Projects/FlameGraph
...
# add some blur
if [ "$TERM" != "linux" ]; then
	blur=_KDE_NET_WM_BLUR_BEHIND_REGION
	konsolex=$(qdbus | grep konsole | cut -f 2 -d\ )

	if [ -n "$konsolex" ]; then
		for konsole in `xdotool search --class konsole`; do
			xprop -f $blur 32c -set $blur 0 -id $konsole;
		done
	fi

	if [ `qdbus | grep yakuake` ]; then
		xprop -f $blur 32c -set $blur 0 -id `xdotool search --class yakuake`;
	fi
fi
